...
class _SafeWebSocketHandler(WebSocketHandler):
    """Custom handler that logs e mensagens inválidas."""

    def read_next_message(self):
        try:
            super().read_next_message()
        except UnicodeDecodeError as exc:
            print(
                f"[NaveMae] Payload WebSocket inválido de {self.client_address}: {exc}"
            )
            self.keep_alive = 0
...
class NaveMae:
    ...
    def start_ws_server(self, host: str = "0.0.0.0", port: int = 2900):
        ...
        server.RequestHandlerClass = _SafeWebSocketHandler
